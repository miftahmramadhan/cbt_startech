  ...
  default:
    ...
    echo "<table class='gtable sortable'>
			...
			";
			  ...
			  $query=mysql_query($pilih);
			  while($row=mysql_fetch_array($query)){
			  	echo"<tr><td><a href=?module=quiz&act=daftarkn&id=$row[id_field]>".$row[knowledge]."</a></td></tr>";
			  }
	...
	echo "<table class='gtable sortable'>
		    ...
		    $query2=mysql_query($pilih2);
		    while($row2=mysql_fetch_array($query2)){				
			echo"<tr><td><a href=?module=quiz&act=daftarpr&id=$row2[id_step]>".$row2[process]."</a></td></tr>";
		   }
	echo"
    ...
    echo "<table class='gtable sortable'>
			...
			";
			  ...
			  $query=mysql_query($pilih);
			  while($row=mysql_fetch_array($query)){
			  	echo"<tr><td><a href=?module=quiz&act=knowledge&id=$row[id_field]>".$row[knowledge]."</a></td></tr>";
			  }
	...
	echo "<table class='gtable sortable'>
		    ...
		    $query2=mysql_query($pilih2);
		    while($row2=mysql_fetch_array($query2)){				
			echo"<tr><td><a href=?module=quiz&act=process&id=$row2[id_step]>".$row2[process]."</a></td></tr>";
		   }
	echo"
...
case "knowledge":
	...
	if ($_SESSION[leveluser]=='siswa'){
        ...
        echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>
            <form method=POST action='soal.php?act=knowledge'>
            <input type=hidden name='waktu' value='6000'>
            ...
            3. Pilih browser yang suport dengan CBT STARTECH.ID yaitu Mozilla Firefox.<br>
            4. Jika mati lampu hubungi Pengajar Mata Pelajaran terkait untuk bisa Ujian Kembali.</h3><br>";
        echo"<p class='garisbawah'></p>
            <input type=submit class='tombol' value='Mulai Mengerjakan' >
...
case "process":
	...
	if ($_SESSION[leveluser]=='siswa'){
        ...
        $t = mysql_fetch_array($topik);
	    echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>
            <form method=POST action='soal.php?act=process'>
            <input type=hidden name='waktu' value='6000'>
            <input type=hidden name='id' value='$_GET[id]'>
            <h3>Baca dengan seksama dan teliti sebelum mengerjakan Tugas / Quiz<p class='garisbawah'></p><b>
            1. Pastikan koneksi anda terjamin dan bagus, misalnya Warnet.<br>
...
case "dnprocess":
 ...
 if ($_SESSION[leveluser]=='siswa'){
        ...
        $pilganda = mysql_query("SELECT * FROM nilai WHERE id_tq = '$_GET[id]' AND id_siswa = '$_SESSION[idsiswa]'");
        $cek_pilganda = mysql_num_rows($pilganda);
                ...
                elseif (empty($cek_pilganda)){
                    echo"<br><b class='judul'>Nilai Anda</b><br><p class='garisbawah'></p>
                      <table>
                      <tr><th>Deskripsi Tugas/Quiz </th><th>Nilai</th></tr>";
                      echo "<tr><td>Soal Pilihan Ganda</td><td>Anda belum mengerjakan</td></tr>
                            </table>
                            <p class='garisbawah'></p><input type=button class='tombol' value='Kembali' onclick=self.history.back()>";
                }
 }
...
case "daftarkop":
	if($_GET[kop]=="kn"){
	 ...
	 if ($_SESSION[leveluser]=='admin'){
        $cek = mysql_query("SELECT COUNT(quiz_pilganda.id_quiz) as jml FROM quiz_pilganda WHERE $varquery = '$_GET[id]'");
        $c = mysql_fetch_array($cek);
        if ($c[jml] != 0){
        $quiz=mysql_query("SELECT * FROM quiz_pilganda WHERE $varquery = '$_GET[id]'");
        $jquiz = mysql_query("SELECT * FROM quiz_pilganda WHERE $varquery = '$_GET[id]'");
        $jq = mysql_fetch_array($jquiz);
        $topik = mysql_query("SELECT * FROM knowledge WHERE $varquery = '$jq[$varquery]'");
        $t=mysql_fetch_array($topik);
        ...
        echo "<div class='information msg'><b>Daftar Soal Tugas/Quiz Pilihan Ganda</b></div>
              ...
              <dl class='inline'>
              <dt><label>Judul</label></dt>          <dd>: $t[$varkop]</dd>
              </dl></fieldset></form>";
        echo "<input type=button class='button blue' value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    echo "<br><br><table id='table1' class='gtable sortable'>";
    $no=1;
    while ($q=mysql_fetch_array($quiz)){
       echo "<tr><td rowspan=9><b>$no.</b></td>
		    ...
		    <div>
             ...
             <tr><td>Knowledge</td><td>: $q[id_field]</td></tr>
             <tr><td>Process</td><td>: $q[id_step]</td></tr>
             ";
      $no++;
    }
    echo "</table>";
    echo "<br><input type=button class='button blue' value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    }else{
...
case "daftarsiswaudahkop":
	if ($_SESSION[leveluser]=='admin'){
        ...
        if (!empty($cek_siswa)){
			...
			Hanya jawaban soal Essay yang bisa di koreksi.</div>";
			...
			$no=1;
			while ($t=mysql_fetch_array($siswa_yangmengerjakan)){
				...
				$n2 = mysql_fetch_array($nilai2);
				  echo "<tr>
						  ...
						  <td>Nilai : $n2[persentase]</td>
						  <td><a href=$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]&id_rvjaw=$n2[id_rvjaw]>Hapus Siswa</a></td>
						</tr>";
            ...
            echo "<tr><td>$no</td>
                      ...
                      if ($t[dikoreksi]=='B'){
                          ...
                          echo "<td>Jawaban soal essay <b>sudah di koreksi</b><br>Nilai Tugas/Quiz Pilihan Ganda : $n2[persentase]<br>
                                                         Nilai Tugas/Quiz Essay: $n[nilai]</td>";
                          echo "
                          <td><a href=javascript:confirmdelete('$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]&id_rvjaw=$n2[id_rvjaw]')>Hapus Siswa</a> |
                          <a href=?module=quiz&act=editkoreksi&id=$t[id_tq]&id_siswa=$s[id_siswa]>Edit Koreksi</a></td></tr>";
                      }
            $no++;
        }
...
case "daftartopik":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Tugas / Quiz Training $data_mapel[nama]</b><br><p class='garisbawah'></p>
              <table>
...
case "daftarnilai":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Nilai Tugas / Quiz Training $data_mapel[nama]</b><br><p class='garisbawah'></p>
              <table>
              <tr><th>No</th><th>Deskripsi Tugas/Quiz </th><th></th></tr>";
              $no=1;
               while($t=mysql_fetch_array($topik)){
                  ...
                  echo"<tr><td rowspan=6>$no</td><td><b>Judul</b></td><td><b>: $t[judul]</b></td></tr>
                       ...
                       $qrvjaw = mysql_query("SELECT id_rvjaw FROM nilai WHERE id_tq='$t[id_tq]' AND id_siswa='$_SESSION[idsiswa]'");
                       $qrv = mysql_fetch_array($qrvjaw);
                       echo"<tr><td><b>Waktu Pengerjaan</b></td><td><b>: $waktu menit</b></td></tr>
                            <tr><td><b>Info Soal/Quiz</b></td><td><b>: $t[info]</b></td></tr>
                            <tr><td></td><td><input type=button class='tombol' value='Lihat Nilai'
                       onclick=\"window.location.href='?module=quiz&act=nilaisiswa&id_topik=$t[id_tq]';\">
                       <input type=button class='tombol' value='Lihat Jawaban'
                       onclick=\"window.location.href='?module=quiz&act=reviewjawaban&idrvjaw=$qrv[id_rvjaw]';\"></td></tr>";
        ...
        $cek_esay = mysql_num_rows($esay);
                ...
                elseif (!empty($cek_pilganda) AND empty($cek_esay)){
                    echo"<br><b class='judul'>Nilai Anda</b><br><p class='garisbawah'></p>
                      ...
                      $n_esay = mysql_fetch_array($esay);
                      echo "<tr><td>Soal Pilihan Ganda</td><td>$n_pilganda[persentase]</td></tr>
                            <tr><td>Soal Essay</td><td>Belum di koreksi</td></tr>
                            </table>
                            <p class='garisbawah'></p><input type=button class='tombol' value='Kembali' onclick=self.history.back()>";
                }
                elseif (empty($cek_pilganda) AND empty($cek_esay)){
                    echo"<br><b class='judul'>Nilai Anda</b><br><p class='garisbawah'></p>
                      ...
                      $n_esay = mysql_fetch_array($esay);
                      echo "<tr><td>Soal Pilihan Ganda</td><td>Anda Belum mengerjakan</td></tr>
                            <tr><td>Soal Essay</td><td>Anda Belum mengerjakan</td></tr>
                            </table>
                            <p class='garisbawah'></p><input type=button class='tombol' value='Kembali' onclick=self.history.back()>";
                }
...
case "reviewjawaban":
	if ($_SESSION[leveluser]=='siswa'){
        $cek = mysql_query("SELECT COUNT(rvjawaban.id_quiz) as jml FROM rvjawaban WHERE id_rvjaw = '$_GET[idrvjaw]'");
        $c = mysql_fetch_array($cek);
        if ($c[jml] != 0){
        $quiz=mysql_query("SELECT * FROM rvjawaban LEFT JOIN quiz_pilganda on rvjawaban.id_quiz=quiz_pilganda.id_quiz where rvjawaban.id_rvjaw='$_GET[idrvjaw]'");
        //$jquiz = mysql_query("SELECT * FROM rvjawaban WHERE id_rvjaw = '$_GET[idrvjaw]'");
        //$jq = mysql_fetch_array($jquiz);
        //$topik = mysql_query("SELECT topik_quiz.id_tq,topik_quiz.judul,nilai.id_tq FROM nilai LEFT JOIN topik_quiz ON topik_quiz.id_tq=nilai.id_tq WHERE nilai.id_rvjaw='$_GET[idrvjaw]'");
        $topik=mysql_query("SELECT * FROM nilai WHERE id_rvjaw='$_GET[idrvjaw]'");
        $t=mysql_fetch_array($topik);
        $topikk=mysql_query("SELECT knowledge AS judul FROM knowledge WHERE id_field='$t[id_tq]'");
        $ctk=mysql_num_rows($topikk);
        $topikp=mysql_query("SELECT process AS judul FROM process WHERE id_step='$t[id_tq]'");
        $ctp=mysql_num_rows($topikp);
        $topikq=mysql_query("SELECT judul FROM topik_quiz WHERE id_tq='$t[id_tq]'");
        $ctq=mysql_num_rows($topikq);
        if (!empty($ctk)){
			$t=mysql_fetch_array($topikk);
		}elseif (!empty($ctp)){
			$t=mysql_fetch_array($topikp);
		}elseif (!empty($ctq)){
			$t=mysql_fetch_array($topikq);	
		}else{
			$t[judul]="Tidak ada Judul";
		}
        //$kelas = mysql_query("SELECT * FROM kelas WHERE id_kelas = '$t[id_kelas]'");
        //$k=mysql_fetch_array($kelas);
        //$pelajaran = mysql_query("SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$t[id_matapelajaran]'");
        ...
        echo "<div class=''><br><br><b class='judul'>Review Jawaban</b></div>
              <form><fieldset>
              <legend>Info Tugas/Quiz</legend>
              <dl class=''>
              <dt><label>Judul : $t[judul]</label></dt> <dd></dd>
              </dl></fieldset></form>";
        echo "<input type=button class='tombol' value=Kembali onclick=\"window.location.href='?module=nilai';\">";
    echo "<br><br><table id='table1' class='gtable sortable'>";
    $no=1;
    while ($q=mysql_fetch_array($quiz)){
       echo "<tr><td rowspan=8><b>$no.</b></td>
		    ...
		    <div>
             ...
             <tr><td>Pilihan D</td><td>: $q[pil_d]</td></tr>
             <tr><td>Kunci Jawaban</td><td>: $q[kunci]</td></tr>
             <tr><td>Jawaban</td><td>: $q[jaw]</td></tr>
             ";
      $no++;
    }
    echo "</table>";
    echo "<br><input type=button class='tombol' value=Kembali onclick=\"window.location.href='?module=nilai';\">";
    }else{
            echo "<script>window.alert('Review Jawaban belum ada, Anda mungkin belum mengerjakannya');
            window.location=(href='?module=nilai')</script>";
        }
    }
...
case "infokerjakan":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>
            <form method=POST action='soal.php?act=bytq' target='_blank'>
...
case "buatquizesay":
    if ($_SESSION[leveluser]=='admin'){
        ...
        <dl class='inline'>
        <dt><label>Pertanyaan ".$jumlah." </label></dt> <dd><textarea name='pertanyaan' id='wysiwyg' cols='50' rows='3'></textarea></dd>
...
case "buatquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo"<fieldset>
             ...
             <dt><label>Pilihan A </label></dt>        <dd><textarea name='pila' id='wysiwyg2' cols='50' rows='3'></textarea></dd>
             
             <dt><label>Pilihan B </label></dt>        <dd><textarea name='pilb' id='wysiwyg' cols='50' rows='3'></textarea></dd>
             
             <dt><label>Pilihan C </label></dt>        <dd><textarea name='pilc' id='wysiwyg' cols='50' rows='3'></textarea></dd>
             
             <dt><label>Pilihan D </label></dt>        <dd><textarea name='pild' id='wysiwyg' cols='50' rows='3'></textarea></dd>
             
             <dt><label>Kunci Jawaban</dt>  <dd><label><input type=radio name='kunci' value=A>A</input><label>
              ...
              echo "                              
              <dt><label>Knowledge</label></dt>              <dd> <select name='id_field'>
                                          <option value=''>-pilih-</option>";
                                          $pilih="SELECT * FROM knowledge ORDER BY id";
                                          $query=mysql_query($pilih);
                                          while($row=mysql_fetch_array($query)){
                                          echo"<option value='".$row[id_field]."'>".$row[knowledge]."</option>";
										  }
										  echo"</select></dd>";
              echo "
              <dt><label>Process</label></dt>              <dd> <select name='id_step'>
                                          <option value=''>-pilih-</option>";
                                          $pilih2="SELECT * FROM process ORDER BY id_step";
                                          $query2=mysql_query($pilih2);
                                          while($row2=mysql_fetch_array($query2)){
                                          echo"<option value='".$row2[id_step]."'>".$row2[process]."</option>";
                                          }
                                          echo"</select></dd>";
			echo "
        ...
        echo"<fieldset>
             ...
             <dt><label>Kunci Jawaban</td>  <dd><label><input type=radio name='kunci' value=A>A</input><label>
              ...
              <dt><label>Knowledge</label></dt>              <dd> <select name='id_field'>
                                          <option value=''>-pilih-</option>";
                                          $pilih="SELECT * FROM knowledge ORDER BY id_field";
                                          $query=mysql_query($pilih);
                                          while($row=mysql_fetch_array($query)){
                                          echo"<option value='".$row[id_field]."'>".$row[knowledge]."</option>";
										  }
...
case "daftarquizpilganda":
    ...
    $no=1;
    while ($q=mysql_fetch_array($quiz)){
       echo "<tr><td rowspan=10><b>$no.</b></td>
             <td><b>Pertanyaan</b></td><td>: $q[pertanyaan]</td></tr>";
             if (empty($q[gambar])){
                 echo "<tr><td>Gambar</td><td>: Tidak ada gambar.</td></tr>";
             }else{
...
case "editquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        ...
        <dl class='inline'>
          <dt><label>Pertanyaan </label></dt> <dd><textarea name='pertanyaan' id='wysiwyg' cols='50' rows='3'>$q[pertanyaan]</textarea></dd>
             <dt><label>Gambar</label></dt>  <dd>";
                    if ($q[gambar]!=''){
              echo "<ul class='photos sortable'>
                    <li>
		    ...
		    <div>
             ...
             <dt><label>Pilihan A </label></dt>        <dd><textarea name='pila' id='wysiwyg2' cols='50' rows='3'>$q[pil_a]</textarea></dd>

             <dt><label>Pilihan B </label></dt>        <dd><textarea name='pilb' id='wysiwyg2' cols='50' rows='3'>$q[pil_b]</textarea></dd>

             <dt><label>Pilihan C </label></dt>        <dd><textarea name='pilc' id='wysiwyg2' cols='50' rows='3'>$q[pil_c]</textarea></dd>

             <dt><label>Pilihan D </label></dt>        <dd><textarea name='pild' id='wysiwyg2' cols='50' rows='3'>$q[pil_d]</textarea></dd>
             <dt><label>Kunci     </label></dt>        <dd>: ";
                    ...
                    $kn=mysql_query("SELECT knowledge FROM knowledge WHERE id_field='$q[id_field]'");
                    $kn=mysql_fetch_array($kn);
                    $kn=$kn[knowledge];
                    $pr=mysql_query("SELECT process FROM process WHERE id_step='$q[id_step]'");
                    $pr=mysql_fetch_array($pr);
                    $pr=$pr[process];
              echo"
                        <dt><label>Knowledge</label></dt>              <dd> <select name='id_field'>
                                          <option value='".$q[id_field]."'> $kn </option>";
                                          $pilih="SELECT * FROM knowledge ORDER BY id";
                                          $query=mysql_query($pilih);
                                          while($row=mysql_fetch_array($query)){
                                          echo"<option value='".$row[id_field]."'>".$row[knowledge]."</option>";
              ...
              echo "
						<dt><label>Process</label></dt>              <dd> <select name='id_step'>
                                          <option value='".$q[id_step]."'> $pr </option>";
                                          $pilih2="SELECT * FROM process ORDER BY id_step";
                                          $query2=mysql_query($pilih2);
                                          while($row2=mysql_fetch_array($query2)){
                                          echo"<option value='".$row2[id_step]."'>".$row2[process]."</option>";
