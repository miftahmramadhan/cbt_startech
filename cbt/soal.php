...
<form action="nilai.php?act=<?php echo"$_GET[act]"; ?>" method="post" id="formulir">
...
case "bytq":
		...
		$soal = mysql_query("SELECT * FROM quiz_pilganda where id_tq='$_POST[id]' ORDER BY rand() LIMIT 4");
...
case "knowledge":
		...
		for ($i=1;$i<=5;$i++){
			$step="pr"."$i";
			$soal = mysql_query("SELECT * FROM quiz_pilganda where id_field='$_POST[id]' and id_step='$step' ORDER BY rand() LIMIT 20");
//			$pilganda = mysql_num_rows($soal);
...
//			}
			...
			//echo $jumlahsoal;
			if ($jumlahsoal == 0){
				echo "<script>window.alert('Maaf belum ada soal di bidang Knowledge Ini.');
							window.location=(href='media.php?module=home')</script>";
			}	
			echo "<input type=hidden name=jumlahsoalpilganda value=$jumlahsoal>";
...
case "process":
		...
		for ($i=1;$i<=10;$i++){
			...
			$soal = mysql_query("SELECT * FROM quiz_pilganda where id_step='$_POST[id]' and id_field='$field' ORDER BY rand() LIMIT 10");
//			$pilganda = mysql_num_rows($soal);
...
//			}
			...
			$jumlahsoal = $no - 1;
			if ($jumlahsoal == 0){
				echo "<script>window.alert('Maaf belum ada soal di bidang process Ini.');
							window.location=(href='media.php?module=home')</script>";
			}	
			echo "<input type=hidden name=jumlahsoalpilganda value=$jumlahsoal>";
